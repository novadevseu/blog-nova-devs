...
function PostContainer({ id, setError }: PostContainerInterface) {
  ...
  // Al cargarse el post, se busca el username del autor usando el uid almacenado en post.author
  useEffect(() => {
    const fetchAuthorUsername = async () => {
      if (post && post.author) {
        try {
          const userDocRef = doc(db, "users", post.author);
          const userDocSnap = await getDoc(userDocRef);
          if (userDocSnap.exists()) {
            const userData = userDocSnap.data();
            setAuthorUsername(userData.username || "Unknown");
          } else {
            console.warn("No se encontró el documento del autor:", post.author);
          }
        } catch (err) {
          console.error("Error al obtener el username del autor:", err);
        }
      }
    };
  ...
  return (
    <article
      className="p-8 rounded-lg w-full max-w-4xl mx-auto bg-[#090d1f]/50 backdrop-blur-sm 
    ...
    >
      ...
      <div className="mb-8 p-6 bg-[#0c1023] rounded-xl border border-gray-800/50">
        <div className="flex flex-wrap gap-4 text-sm text-gray-400">
          <div className="flex items-center gap-2">
            ...
            <span>{authorUsername || post?.author || "Anónimo"}</span>
          </div>
          <div className="flex items-center gap-2">
            <span className="w-2 h-2 bg-[#E0C600] rounded-full"></span>
      ...
      {/* Content */}
      <div
        className="markdown-content mb-8 prose prose-invert prose-yellow 
